...
const world = World(
    init(problemp, AutoTsit5(Rosenbrock23()); save_everystep = false, callback = cback),
    0.0,
    0.0,
)
...
const controls = Controls(0.0, 0.0, false)
...
function Gloria.render!(L::Layer, obj::World, frame, fps)
    ...
    for x in obj.Integ.u.nodes
        i += 1
        Gloria.render!(L, circle(Vertex(0.0, 0.0), 50; samples = i), 800 + x[1], 450 + x[2])
    end
